...
## approach used
# #Hold-out testing
# X_train, X_test, y_train, y_test = train_test_split(matrix, labels, test_size=0.20)
...
mlp1 = MLPClassifier(activation = 'identity',
                        ...
                        n_iter_no_change = 10,
                        verbose = True)
mlp1.fit(matrix,labels)
predictions = mlp1.predict(eval_matrix)
...
with open ('out2.csv', 'w') as f:
    ...
    for p in predictions:
        f.write("%d,%s\n" %(i,p))
        i += 1
print("results written to out2.csv")
